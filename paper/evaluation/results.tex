...
used $\eta$ time series normalization, DTW with a Sakoe-Chiba band of about 36 \% time series length, \textit{Mid} window size, and \textit{HAvgD} for threshold determination.
...
For this experiment we selected only the dominating parameter settings, with $\eta$ normalization, \textit{Mid} window size, and and \textit{HAveD} dissimilarity threshold. The best $F_{1}score_{\mu}$ was achieved with a band with of 36 \% time series length.
...
We evaluate three different ways of determining a dissimilarity threshold, namely \textit{HMinD}, \textit{HAveD}, and \textit{HMidD}. For our comparison in Figure \ref{fig:result}(c), we used $\eta$ normalization, \textit{Mid} window size, and a warping band of 36 \% time series length. The best $F_{1}score_{\mu}$ was achieved by means of \textit{HAveD}, shortly followed by the \textit{HMidD} approach. Comparatively high $Precision_{\mu}$ values were given by \textit{HMinD} threshold.


\paragraph{Window Size:} 

The influence of the window size determination approach is shown in Figure \ref{fig:result}(d). We compare \textit{Min}, \textit{Max}, \textit{Ave}, and \textit{Mid} window size, with $\eta$ normalization, \textit{HAveD} dissimilarity threshold, and a warping and of 36 \% time series length. The highest $Precision_{\mu}$, $Recall_{\mu}$ and $F_{1}score_{\mu}$ was achieved by the \textit{Mid} window size, shortly followed by the \textit{Ave} window size. Figure \ref{fig:result}(d) furthermore suggests to refrain from using \textit{Max} and \textit{Min} window size determination.
