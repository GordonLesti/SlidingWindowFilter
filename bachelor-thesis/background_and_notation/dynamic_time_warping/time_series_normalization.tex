...
\begin{center}
    $\eta '(q) = SMULT(\frac{1}{\sigma}, \eta (q))$
\end{center}
where $\sigma$ is the standard deviation of the time series $Q$
\begin{center}
    $\sigma = SMULT(\frac{1}{l-1}, \sum \limits_{i=1}^{l} D(q_i, \bar{q})^2)$
\end{center}
Again is the sigma sum sign $\sum$ in the last formula used under the context of the $ADD$ function between elements of
$\mathbb{U}$. Figure \ref{fig:norm2} shows the impact of the $\eta '$ normalization on DTW compared to plain DTW in
